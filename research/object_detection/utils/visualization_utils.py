...
def visualize_boxes_and_labels_on_image_array_big_box(
    image,
    boxes,
    classes,
    scores,
  ...
  # Draw all boxes onto image.
  if len(box_to_color_map.items()) > 0:
      ymin_big = min(map(lambda x: x[0][0],box_to_color_map.items()))
      xmin_big = min(map(lambda x: x[0][1],box_to_color_map.items()))
      ymax_big = max(map(lambda x: x[0][2],box_to_color_map.items()))
      xmax_big = max(map(lambda x: x[0][3],box_to_color_map.items()))
      draw_bounding_box_on_image_array(
          image,
          ymin_big,
          xmin_big,
